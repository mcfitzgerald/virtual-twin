...
from virtual_twin.cli.configure import configure as configure_func
from virtual_twin.cli.simulate import simulate as simulate_func
...
def main():
    ...
    # This enables backward compatibility: python -m virtual_twin --run baseline_8hr
    if (
        len(sys.argv) > 1
...
Examples:
  ...
  python -m virtual_twin run --run baseline_8hr
  python -m virtual_twin --run baseline_8hr  # shorthand (legacy mode)

  # Generate scenario bundle, then run it
  python -m virtual_twin configure --run baseline_8hr
  python -m virtual_twin simulate --scenario scenarios/baseline_8hr_20250126_143022
  ...
  python -m virtual_twin run --run baseline_8hr --export
    ...
    run_parser.add_argument(
        "--db-path",
        default=None,
        help="Custom path for DuckDB file (default: ./virtual_twin_results.duckdb)",
    )
    run_parser.add_argument(
        "--debug-events",
        action="store_true",
